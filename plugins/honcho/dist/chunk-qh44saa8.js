...
var DEDUP_WINDOW_TURNS = 8;
var DEDUP_FLOOR = 3;
function dedupKey(conclusion, namespace) {
...
function filterRepeats(conclusions, ledger, namespace) {
  if (conclusions.length === 0)
    return { kept: [], suppressed: 0, floored: false };
...
async function handleUserPrompt() {
  ...
  const errorLines = [userCtxResult?.error, dialecticResult?.error].filter((e) => !!e).map((e) => visErrorLine("user-prompt", e));
  const diagnostics = getCurrentOutputLevel() === "verbose" ? visDiagnostics(config, {
    retrieval: `${(elapsedMs / 1000).toFixed(1)}s`,
    components: injection.perTurn.join(", ") || "none",
    conclusions: (userCtx ? extractConclusions(userCtx.context).length : 0) + (assistantCtx ? extractConclusions(assistantCtx).length : 0),
...
function emitPerTurn(config, injection, userCtx, assistantCtx, sessionCtx, dialectic, opts = {}) {
  const parts = [];
  const visLines = [];
  const show = (c) => injection.showContents?.includes(c) ?? false;
  const dedup = (conclusions, namespace) => opts.dedup ? filterRepeats(conclusions, opts.dedup, namespace).kept : conclusions;
  if (userCtx) {
...
`);
  if (parts.length === 0) {
    if (visMsg)
      console.log(JSON.stringify({ systemMessage: visMsg }));
    return;
  }
...
export { isTerseReply, isHarnessInjected, handleUserPrompt };

//# debugId=9FCB0ACD42C671B364756E2164756E21
//# sourceMappingURL=chunk-qh44saa8.js.map
