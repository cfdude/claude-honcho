...
function isGitRepo(cwd) {
  return existsSync(join2(cwd, ".git"));
}
...
function idCacheFile() {
  return join3(cacheDir(), "cache.json");
}
function contextCacheFile() {
  return join3(cacheDir(), "context-cache.json");
}
...
function loadContextCache() {
  ensureCacheDir();
  if (!existsSync2(contextCacheFile())) {
    return {};
  }
  try {
...
function gitStateFile() {
  return join3(cacheDir(), "git-state.json");
}
function loadGitStateCache() {
  ensureCacheDir();
  if (!existsSync2(gitStateFile())) {
    return {};
  }
  try {
...
function findProjectConfig(cwd, stopDir = homeDirPath()) {
  ...
  while (true) {
    if (dir === stopDir)
      break;
    const candidate = join4(dir, ".honcho.json");
...
function getPluginVersion() {
  ...
  const candidates = [
    ...root ? [join5(root, ".claude-plugin", "plugin.json")] : [],
    fileURLToPath(new URL("../.claude-plugin/plugin.json", import.meta.url))
  ];
  for (const manifest of candidates) {
    try {
      const version = JSON.parse(readFileSync3(manifest, "utf-8")).version;
...
function saveConfig(config, cwd = process.cwd()) {
  ...
  if (existsSync4(configFile)) {
    try {
      existing = JSON.parse(readFileSync3(configFile, "utf-8"));
    } catch {}
  }
...
export { __toESM, __commonJS, __export, __require, captureGitState, homeDirPath, honchoDir, findProjectConfig, getProjectWorkspace, SESSION_START_COMPONENTS, PER_TURN_COMPONENTS, normalizePerTurn, REASONING_LEVELS, OUTPUT_LEVELS, DEFAULT_OUTPUT_LEVEL, parseOutputLevel, setDetectedHost, getDetectedHost, coerceBoolean, getCachedStdin, readStdinText, initHook, getConfigPath, configExists, getPluginVersion, loadConfig, getWorkspaceProvenance, saveConfig, saveRootField, getSessionForPath, getSessionName, setSessionForPath, getInjectionConfig, isLoggingEnabled, isPluginEnabled, getKnownHosts, getHonchoClientOptions, getEndpointInfo, getObservationMode, setCachedSessionId, getLastActiveCwd, getClaudeInstanceId, setClaudeInstanceId, getInstanceIdForCwd, incrementMessageCount, getMessageCount, resetMessageCount, getCachedGitState, setCachedGitState, detectGitChanges, chunkContent, addMessagesBatched, clearIdCache, clearPeerCache, clearUserContextOnly, clearClaudeContextOnly };

//# debugId=66CC7B31C66DCE7D64756E2164756E21
//# sourceMappingURL=chunk-er0jc8ja.js.map
