...
// FORK: hooks.json invokes every hook through run-hook.sh rather than calling
// the runtime directly, so the wrapper has to exist in the stage or
// assertStagedPaths() below rejects the build. See rewriteEntryPoints().
await cp(join(ROOT, "scripts/run-hook.sh"), join(STAGE, "scripts/run-hook.sh"));
...
function rewriteEntryPoints(text: string): string {
  return text
    ...
    // FORK: this fork routes hooks through scripts/run-hook.sh, which resolves
    // the runtime and fails LOUDLY when it is missing — a bare `bun`/`node` in
    // hooks.json dies silently on a non-interactive PATH, taking memory capture
    // with it and leaving nothing in the transcript or the plugin's own log.
    // The wrapper keeps its argument; only the target is rewritten to the
    // bundled JS, so the upstream forms above never match it.
    .replace(
      /(run-hook\.sh\\?"?\s+\\?"?)\$\{CLAUDE_PLUGIN_ROOT\}\/(hooks\/[\w-]+)\.ts/g,
      '$1${CLAUDE_PLUGIN_ROOT}/dist/$2.js',
    );
}
